...
<div class="mb-0 ">
  <div class="flex flex-wrap gap-6 w-full">
    ...
    @foreach ($latestUpdatedBooksA  as $book)
<div class="indicator">
  ...
  <div class="card card-side w-200 h-40 bg-transparent backdrop-blur-md ml-14 shadow-sm">
        ...
        <div class="card-body px-6 py-4">
          ...
          <a href="{{ route('books.show-admin', $book->id) }}" class="card-title text-lg text-white font-medium hover:text-base-300">{{ $book->title }}</a>
    ...
    <div class="fixed bottom-8 left-[56%] -translate-x-1/2 z-50">
      <div class="join shadow-lg rounded-xl bg-white/80 backdrop-blur-md px-4 py-2 space-x-1">
        ...
        @if ($latestUpdatedBooksA->onFirstPage())
            <span class="join-item btn btn-xs rounded-md opacity-50 cursor-not-allowed">Prev</span>
        @else
            <a href="{{ $latestUpdatedBooksA->previousPageUrl() }}" class="join-item btn btn-xs rounded-md hover:bg-green-500 hover:text-white">Prev</a>
        @endif

        @for ($i = 1; $i <= $latestUpdatedBooksA->lastPage(); $i++)
            <a href="{{ $latestUpdatedBooksA->url($i) }}"
              class="join-item btn btn-xs rounded-md {{ $latestUpdatedBooksA->currentPage() == $i ? 'bg-green-500 text-white' : '' }}">
                {{ $i }}
            </a>
        @endfor

        @if ($latestUpdatedBooksA->hasMorePages())
            <a href="{{ $latestUpdatedBooksA->nextPageUrl() }}" class="join-item btn btn-xs rounded-md hover:bg-green-500 hover:text-white">Next</a>
        @else
