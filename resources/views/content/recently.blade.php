  ...
  <div class="flex flex-wrap gap-6 w-full">
    ...
    <div class="fixed bottom-8 left-[56%] -translate-x-1/2 z-50">
      <div class="join shadow-lg rounded-xl bg-white/80 backdrop-blur-md px-4 py-2 space-x-1">
        ...
        @if ($recentlyAddedBooks->onFirstPage())
            <span class="join-item btn btn-xs rounded-md opacity-50 cursor-not-allowed">Prev</span>
        @else
            <a href="{{ $recentlyAddedBooks->previousPageUrl() }}" class="join-item btn btn-xs rounded-md hover:bg-green-500 hover:text-white">Prev</a>
        @endif

        @for ($i = 1; $i <= $recentlyAddedBooks->lastPage(); $i++)
            <a href="{{ $recentlyAddedBooks->url($i) }}"
              class="join-item btn btn-xs rounded-md {{ $recentlyAddedBooks->currentPage() == $i ? 'bg-green-500 text-white' : '' }}">
                {{ $i }}
            </a>
        @endfor

        @if ($recentlyAddedBooks->hasMorePages())
            <a href="{{ $recentlyAddedBooks->nextPageUrl() }}" class="join-item btn btn-xs rounded-md hover:bg-green-500 hover:text-white">Next</a>
        @else
